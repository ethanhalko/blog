...
export default function LayoutMain<T extends ReactNode | ReactNode[]>({children, showContent = true}: { children: T, showContent: boolean }) {
  return (
    <>
      <div className="min-h-screen h-full flex flex-col w-full bg-neutral-50 px-8 pt-8 pb-16">
        <div className={`site-header flex flex-col content-center justify-center ${showContent ? '' : 'mt-64'}`}>
          <h1 className={'text-2xl sm:text-3xl md:text-4xl lg:text-5xl text-center mx-auto'}>
            Ethan Halko Dot Com
          </h1>
          <ul className={'flex mx-auto space-x-4 my-6'}>
            {linkNodes}
          </ul>
          {children}
        </div>
